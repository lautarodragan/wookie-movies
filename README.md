...
- People never explore this folder, and there's no reason to. An IDE's `Navigate to Source` is a much more sensible tool. This means the structure of this folder will not have a big impact.
...
- `Layout` is special: it's used by Pages to wrap their contents with UI/UX that is shared across pages, such as navigation menus and the footer. If we had more than one layout, I'd probably place them in their own `layouts` folder.
